...
- `Features/Diagnostics/MCP`: hidden DEBUG MCP diagnostics, transport diagnostics, Sparkle diagnostics, and memory sampling exposed through `__repoprompt_debug_diagnostics` / legacy debug transport tools. The read-file invocation packet assembler lives here because it field-picks bounded, path-free evidence from existing diagnostics authorities; operators discover captured invocation IDs with `list_read_file_invocations` and request one partial or complete packet with `read_file_invocation_packet`.
