...
final class MCPServerViewModel: ObservableObject {
    struct CodeStructureSettlementLimitNotice: Equatable {
        /// Generated by the network manager. The view model only renders this
        /// projection and rejects delayed updates from older generations.
        let generation: UInt64

        var title: String {
            "MCP file and code tools are unavailable"
        }

        var message: String {
            "An earlier MCP request did not stop. Wait for it to finish, or restart RepoPrompt CE to restore these tools."
        }
    }
    ...
    /// A per-window operational warning owned by the structure-settlement lease.
    /// It is intentionally separate from tool-registration failures because the
    /// catalog remains registered while a cancellation-ignoring provider settles.
    @Published private(set) var codeStructureSettlementLimitNotice: CodeStructureSettlementLimitNotice?
    /// Retains the highest generation after dismissal, preventing an older
    /// main-actor presentation queued before recovery from reappearing later.
    private var codeStructureSettlementLimitNoticeGenerationTombstone: UInt64 = 0
    private var windowToolsRequested = false
    var windowToolsAreRequested: Bool {
        windowToolsRequested
    }

    func presentCodeStructureSettlementLimitNotice(generation: UInt64) {
        guard generation > codeStructureSettlementLimitNoticeGenerationTombstone else { return }
        codeStructureSettlementLimitNoticeGenerationTombstone = generation
        codeStructureSettlementLimitNotice = CodeStructureSettlementLimitNotice(generation: generation)
    }

    /// The manager clears only the generation it owns. Recording the clear even
    /// before its presentation arrives keeps a delayed older update dismissed.
    func clearCodeStructureSettlementLimitNotice(generation: UInt64) {
        guard generation >= codeStructureSettlementLimitNoticeGenerationTombstone else { return }
        codeStructureSettlementLimitNoticeGenerationTombstone = generation
        guard (codeStructureSettlementLimitNotice?.generation ?? 0) <= generation else { return }
        codeStructureSettlementLimitNotice = nil
    }
